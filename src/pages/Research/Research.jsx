...
import { FlaskConical, Calendar, User, Plus } from 'lucide-react';
...
const Research = () => {
    ...
    const { currentUser, canPublishResearch } = useAuth();
    const [openDialog, setOpenDialog] = useState(false);
    const [formData, setFormData] = useState({ title: '', description: '', content: '', imageUrl: '' });
    const [uploading, setUploading] = useState(false);

    useEffect(() => {
        const q = query(collection(db, 'research_articles'), orderBy('createdAt', 'desc'));
    ...
    const handleImageUpload = async (e) => {
        ...
        setUploading(true);
        try {
            const storageRef = ref(storage, `research_articles/${Date.now()}_${file.name}`);
        ...
        <Container maxWidth="lg" sx={{ py: 6 }}>
            <Box sx={{ mb: 6, textAlign: 'center' }}>
                ...
                <Typography variant="h6" color="text.secondary" sx={{ maxWidth: 600, mx: 'auto' }}>
                    Explora nuestros últimos hallazgos en tecnología y software.
                </Typography>
            </Box>

            {articles.length === 0 ? (
                <Box sx={{ textAlign: 'center', py: 8 }}>
                    <FlaskConical size={64} color="#dadce0" style={{ marginBottom: '16px' }} />
                    <Typography color="text.secondary">
                        No hay artículos de investigación disponibles aún.
                    </Typography>
                </Box>
            ) : (
            ...
            <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
                <DialogTitle>Publicar Artículo de Investigación</DialogTitle>
                <DialogContent>
